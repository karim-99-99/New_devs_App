...
interface RevenueData {
    property_id: string;
    // Exact decimal string: sending money as a JSON number would put it
    // through IEEE-754 before it ever reaches the browser.
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', showRaw }) => {
    ...
    useEffect(() => {
        const fetchRevenue = async () => {
            setLoading(true);
            try {
                // The tenant comes from the authenticated session only; a
                // client-supplied tenant would be trivially forgeable.
    ...
    // Already rounded to cents server-side; split the string so the digits
    // shown are the digits the API sent.
    const [wholePart, fractionPart = '00'] = data.total_revenue.split('.');
    const displayTotal = `${Number(wholePart).toLocaleString()}.${fractionPart.padEnd(2, '0').slice(0, 2)}`;
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div className="p-6">
                ...
                {/* Precision Warning Area: fires if the exact string the API sent
                    cannot survive a round-trip through a JS number. */}
                <div className="mt-4 h-6">
                    {Number(data.total_revenue).toFixed(2) !== data.total_revenue && showRaw && (
