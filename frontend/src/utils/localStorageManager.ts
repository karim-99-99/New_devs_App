...
class LocalStorageManager {
  ...
  private migrateV1ToV2(): void {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.includes('supabase') || key.includes('sb-') || key === 'base360-auth-token')) {
          const value = localStorage.getItem(key);
          if (value) {
            preservedData[key] = value;
            console.log(`Preserving auth/session key: ${key}`);
          }
        }
      }
  ...
  private clearAllExceptEssentials(): void {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      // 'base360-auth-token' is the local auth client's session key; wiping it
      // logs the user out on the next page load.
      if (key && (key.includes('supabase') || key.includes('sb-') || key === 'base360-auth-token')) {
        const value = localStorage.getItem(key);
        if (value) {
          preservedData[key] = value;
