  ...
  private checkForCorruption(): StorageIssue[] {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        ...
        // Not JSON at all. Plain-string values (e.g. app_storage_version,
        // i18nextLng) are legitimate, so they must not be deleted as
        // "corrupted" — removing app_storage_version made the next page load
        // run the null-version migration, which wiped the auth session.
        continue;
      }
    }
